...
void salvarFrequencias(Frequencia fqs[], int qtd) {
    ...
    if(!f){ printf("Erro ao salvar frequ�ncias.\n"); return; }
    write_bom_if_file_empty(f);
    for(int i=0;i<qtd;i++)
        fprintf(f,"%s;%s;%s;%s;%s\n",
...
void listarUsuarios(Usuario usuarios[], int qtdUsuarios) {
    printf("\n--- Usu�rios ---\n");
    if (qtdUsuarios==0) { printf("Nenhum usu�rio.\n"); return; }
...
void diarioEletronico(const Usuario *prof, Usuario usuarios[], int qtdUsuarios, Turma turmas[], int qtdTurmas) {
    ...
    printf("Presen�as registradas com sucesso!\n");
}
...
void menuAdmin(Usuario usuarios[], int *qtdUsuarios, Turma turmas[], int *qtdTurmas, Atividade atividades[], int *qtdAtividades) {
    ...
    do {
        ...
        printf("3) Cadastrar usu�rio (nome/login/senha/tipo/telefone/email/turma)\n");
        ...
        if (opc==1) {
            ...
            if (*qtdUsuarios >= MAX) { printf("Limite de usu�rios atingido.\n"); continue; }
            Usuario u;
            printf("Nome completo: ");
            fgets(u.nome, NOME_SZ, stdin); u.nome[strcspn(u.nome,"\r\n")]=0;
            printf("Login (�nico): ");
            fgets(u.login, LOGIN_SZ, stdin); u.login[strcspn(u.login,"\r\n")]=0;
            if (existeLogin(usuarios, *qtdUsuarios, u.login)) { printf("Login j� existe. Cancela.\n"); continue; }
            printf("Senha: ");
            ...
            if (*qtdTurmas == 0) {
                u.turma[0]=0;
                printf("Nenhuma turma cadastrada. Usuario ficar� sem turma.\n");
            } else {
                printf("Escolha turma pelo c�digo (ou deixe em branco):\n");
                listarTurmas(turmas, *qtdTurmas);
                printf("C�digo (ex: A1) ou apenas Enter: ");
                ...
                if (strlen(sel)==0) u.turma[0]=0;
                    ...
                    if (encontrarIndiceTurmaPorCodigo(turmas, *qtdTurmas, sel) == -1) {
                        printf("Turma n�o encontrada. Usu�rio ficar� sem turma.\n");
                        u.turma[0]=0;
                    } else {
...
void menuProfessor(const Usuario *prof, Usuario usuarios[], int qtdUsuarios, Turma turmas[], int qtdTurmas) {
    ...
    do {
        ...
        printf("3) Di�rio eletr�nico (registrar presen�as)\n");
        printf("0) Sair\n");
        printf("Escolha: ");
        scanf("%d", &opc);
        getchar();
        if (opc==1) {
            ...
            if (qtdTurmas==0) {
                printf("Nenhuma turma cadastrada. Atividade n�o criada.\n");
            } else {
                listarTurmas(turmas, qtdTurmas);
                printf("Digite o c�digo da turma para a atividade: ");
                char cod[TURMA_COD_SZ];
                fgets(cod, TURMA_COD_SZ, stdin); cod[strcspn(cod,"\r\n")]=0;
                if (encontrarIndiceTurmaPorCodigo(turmas, qtdTurmas, cod)==-1) {
                    printf("Turma inv�lida. Cancelando.\n");
                } else {
            ...
            if (!found) printf("Voc� n�o tem atividades postadas.\n");
        } else if (opc==3) {
...
int main() {
    ...
    if (qtdUsuarios == 0) {
        ...
        qtdUsuarios = 1;
        salvarUsuarios(usuarios, qtdUsuarios);
        printf("Admin padr�o criado: login=admin senha=1234\n");
    }

    int opcaoSistema;
    do {
        printf("\n=== Sistema Acad�mico (UTF-8) ===\n");
